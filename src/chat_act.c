...
bool chat_command( CHAR_DATA * ch, char * argument ) {
  ...
  if ( arg[ 0 ] == '>' ) {
    ...
    act( num_color( ch ), "$N &Y(to you): &G$t", victim, argument, ch, TO_CHAR );
    act( AT_RED, "-- &CMessage directed to &$t$N &R--", ch, get_color( victim ), victim, TO_CHAR );
    return TRUE;
  }
  ...
  if ( !str_prefix( arg + 1, "quit" ) ) {
    ...
    act( num_color( ch ), "$N &C(&RPriv&C)&Y: &G$t", victim, argument, ch, TO_CHAR );
    act( AT_RED, "-- &CMessage sent only to &$t$N &R--", ch, get_color( victim ), victim, TO_CHAR );
  }
...
void chat_page( CHAR_DATA * ch, char * argument ) {
  ...
  act( num_color( ch ), "$N &C(&RPAGE&C)&Y: &G$t", victim, argument, ch, TO_CHAR );
  act( AT_RED, "-- &CPage sent to &$t$N &R--", ch, get_color( victim ), victim, TO_CHAR );
  return;
}
...
void chat_join( CHAR_DATA * ch, char * argument ) {
  ...
  if ( arg[ 0 ] == '\0' ) {
    ...
    if ( !str_prefix( ch->name, ch->in_room->name ) || is_name( NULL, ch->name, ch->in_room->name ) ) {
      ToMain = TRUE;
    }
    ...
    sprintf( buf, "%s'%s room.", ch->name, ( ch->name[ strlen( ch->name ) - 1 ] == 's' ? "" : "s" ) );
...
void chat_topic( CHAR_DATA * ch, char * argument ) {
  ...
  act( num_color( ch ), "$n has changed the topic to '$t'.", ch, argument, NULL, TO_ROOM );
  return;
}
...
void chat_who( CHAR_DATA * ch, char * argument ) {
  ...
  for ( victim = chat_list; victim; victim = victim->next ) {
    sprintf( buf + strlen( buf ), "&%s%-16s&R%-13s&P%s\n\r", get_color( victim ), victim->name, victim->in_room->name, victim->in_room->description );
    rcount++;
  }
...
void chat_help( CHAR_DATA * ch, char * argument ) {
  send_to_char( AT_LBLUE, "/help                  &GThis help screen\n\r", ch );
  send_to_char( AT_LBLUE, "/join <channel>        &GJoin a channel\n\r", ch );
  send_to_char( AT_LBLUE, "/page <who> <msg>      &GGlobal private message\n\r", ch );
  send_to_char( AT_LBLUE, "/<who> <msg>           &GChannel private message\n\r", ch );
  send_to_char( AT_LBLUE, "><who> <msg>           &GChannel directed message\n\r", ch );
  send_to_char( AT_LBLUE, "/quit                  &GQuit the conference\n\r", ch );
  send_to_char( AT_LBLUE, "/rooms                 &GList active rooms\n\r", ch );
  send_to_char( AT_LBLUE, "/topic <topic>         &GSet room topic\n\r", ch );
  send_to_char( AT_LBLUE, "/who ['game' [name]]   &GList people in conference\n\r", ch );
  send_to_char( AT_LBLUE, "/invite [who]          &GInvite someone to your room\n\r", ch );
  send_to_char( AT_LBLUE, "/uninvite <who>        &GUninvite someone from your room\n\r", ch );
  send_to_char( AT_LBLUE, "/social                &GList all the socials\n\r", ch );
  send_to_char( AT_LBLUE, ".<social>              &GPerform a social\n\r", ch );
  send_to_char( AT_LBLUE, ",<msg>                 &GEmote something\n\r", ch );
  return;
}
...
void chat_invite( CHAR_DATA * ch, char * argument ) {
  ...
  if ( arg[ 0 ] == '\0' ) {
    ...
    for ( pInv = one_argument( pInv, iArg ); iArg[ 0 ] != '\0'; pInv = one_argument( pInv, iArg ) ) {
      width += strlen( iArg ) + 1;
      ncount++;
  ...
  if ( !victim ) {
    ...
    act( AT_PINK, "You have been invited to &$t$N&P's room.", victim, get_color( ch ), ch, TO_CHAR );
  }
